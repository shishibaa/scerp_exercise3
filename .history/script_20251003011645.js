...
function renderList(list) {
    ...
    list.forEach(member => {
        ...
        let websiteHtml = `<a href="#" onclick="alert('The website is under construction');">Visit</a>`;
        if (member.website && isValidHttpUrl(member.website)) {
            const safeHref = escapeHtml(member.website);
            websiteHtml = `<a class="visit-link" href="${safeHref} rel="noopener noreferrer nofollow">Visit</a>`;
        }
        div.innerHTML = `
